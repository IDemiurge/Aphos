package module.battle.features.damage;

import elements.exec.EntityRef;
import elements.content.battle.stats.action.ActionParam;
...
import module.battle.features.rules.damage.ImmuneRule;
...
import static elements.content.battle.types.CombatTypes.RollGrade;
...
public class DamageCalc {
    ...
    private boolean isWardBlock(Damage damage, EntityRef ref) {
        if (ImmuneRule.checkImmune(ref.getTarget(), damage.getType(), ref.getSource())) {
            return true;
        }
        if (WardRule.checkWard(ref.getTarget(), damage.getType(), ref.getSource())) {
            return true;
        }
        return false;
    }
