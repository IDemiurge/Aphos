...
public class WaitMaster {
    ...
    public static void unlock(Object o) {
        ...
        if (lock == null) {
            System.out.println("NO LOCK " + o);
            return;
        }
         System.out.println("UNLOCKING " + o);
        lock.lock();
...
//        main.system.auxiliary.system.log.LogMaster.dev("UNLOCKED " + o);
    ...
    public static void waitLock(Object o, int maxMillis) {
        if (unlocked.contains(o)) {
             System.out.println("ALREADY UNLOCKED " + o);
            return;
        }
        Lock lock = new ReentrantLock();
        if (locks.containsKey(o)) {
             System.out.println("******ALREADY LOCKED " + o);
            return;
        }
        locks.put(o, lock);
        Condition waiting = lock.newCondition();
        conditions.put(lock, waiting);
         System.out.println("LOCKED with key: " + o);
        lock.lock();
        try {
            if (maxMillis > 0) {
    ...
    public static void interrupt(WAIT_OPERATIONS waiter) {
        if (waiters.get(waiter) == null) {
            // SysLog.printOut(1, "No such operation in process!" + waiter.name());
            return;
        }
